...
class LanguageModelPlugins(ABC):
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        if "Abstract" not in cls.__name__:
            print(f"Registering Language model plugin '{cls.__name__}' with flavor '{cls.llm_flavor()}'")
            cls.language_model_plugins[cls.llm_flavor()] = cls
...
class LanguageModelPluginsDiscovery():
    def __init__(self):
        self._setup_logger()
    ...
    def list_files_in_plugin_dir(self, plugins_directory_path: str) -> list[str]:
        ...
        self._logger.info(f"List of Language model plugins found: '{files}'")
        if len(files) == 0:
            self._logger.error(f"No Language model plugins found in directory: '{plugins_directory_path}'")
